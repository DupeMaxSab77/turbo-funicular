    ...
    try:
        with sync_playwright() as p:
            ...
            pg.evaluate("""()=>{
                ...
                new MutationObserver(muts=>{muts.forEach(m=>{m.addedNodes.forEach(n=>{if(n.nodeType===1){const t=(n.className||'')+(n.id||'');if(/popup|interstitial|preroll|overlay-ad|ad-container|video-ad|sponsor/i.test(t))n.remove();if(n.tagName==='IFRAME'){const s=(n.src||'').toLowerCase();if(s.includes('ad')||s.includes('peacock'))n.remove()}}})})}).observe(document.body,{childList:true,subtree:true});
            }""")
            time.sleep(1)
